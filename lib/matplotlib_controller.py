...
class MatplotlibController(BaseController):
  ...
  def __init__(self, animation, animation_kwargs: Dict, n_pixels: int, validate_parameters=True, background_color: str = 'black', show_tree: bool = False):
    ...
    # Add static 3D Christmas tree (optional)
    if show_tree:
  ...
  def _add_christmas_tree(self):
    ...
    for tier in range(num_tiers):
      ...
      # Make tiers touch each other - each tier's top is the next tier's base
      if tier == num_tiers - 1:
        # Top tier extends to a sharp point (taller to stick out over lights)
        tier_top_z = z_max + tree_height * 0.12
      else:
        tier_top_z = z_min + tree_height * (tier + 1) / num_tiers
      ...
      # Make tree slightly smaller than lights to ensure it renders behind them
      tier_base_radius = max_radius * 0.90 * (1 - tier / num_tiers * 0.85)
      ...
      # Push tree surfaces slightly inward to ensure they render behind lights
      # Scale x and y coordinates toward center by a small factor
      push_inward = 0.96  # Push 4% inward to ensure lights are always in front
      x_cone = x_cone * push_inward
      y_cone = y_cone * push_inward

      # Vary green color per tier
      green_val = 0.25 + 0.2 * (tier / num_tiers)
      tier_color = (0.05, green_val, 0.02)

      self.ax.plot_surface(x_cone, y_cone, z_grid, color=tier_color, alpha=1.0, shade=True, antialiased=True, zorder=1)
    ...
    for b in range(num_branches):
      ...
      branch_r = max_radius * 0.85 * (1 - (branch_base_z - z_min) / tree_height) + t * branch_length
      branch_z = branch_base_z - t * branch_length * 0.3
      branch_x = branch_r * np.cos(branch_angle)
      branch_y = branch_r * np.sin(branch_angle)

      self.ax.plot(branch_x, branch_y, branch_z, color=(0.0, 0.35, 0.0), linewidth=2.5, zorder=1)
    ...
    # Push trunk slightly inward to ensure it renders behind lights
    push_inward = 0.96
    x_trunk = x_trunk * push_inward
    y_trunk = y_trunk * push_inward

    self.ax.plot_surface(x_trunk, y_trunk, z_trunk_grid, color='saddlebrown', alpha=1.0, shade=True, zorder=1)
    ...
    star_mesh = Poly3DCollection(faces, alpha=1.0, facecolor='gold', edgecolor='orange', linewidth=0.5, zorder=3)
